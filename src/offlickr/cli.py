...
class _OrderedGroup(click.Group):
    """click.Group subclass that lists commands in registration order."""

    def list_commands(self, ctx: click.Context) -> list[str]:
        return ["build", "ingest", "derive", "fetch-external", "render", "inspect", "serve"]
...
def main(log_level: str) -> None:
    ...
    Quick start: offlickr build SOURCE -o OUTPUT

    Full pipeline: ingest → derive → [fetch-external] → render
    """
    load_dotenv()
...
def ingest(
    ...
    """Parse a Flickr export into a validated archive."""
...
def derive(output_dir: Path, jobs: int) -> None:
    """Generate thumbnails, display images, and search index."""
...
@click.option(
    "--hide-unsafe",
    is_flag=True,
    help="Exclude moderate/restricted photos from the photostream (build-time; shared archives).",
)
@click.option(
...
@click.option(
    "--include-exif-pii",
    is_flag=True,
    help="Include identifying EXIF fields (Artist, Copyright, camera/lens serials) in photo pages.",
)
@click.option(
    "--flickr-origin",
    is_flag=True,
    help=(
        "Enable Flickr-origin mode: show engagement stats (views/faves/comments), "
        "groups, safety notices, linked people tags, linked comment authors, "
        "'View on Flickr' links, the About page, and canonical/noindex meta tags. "
        "Off by default for standalone archives."
    ),
)
def render(
    output_dir: Path,
    ...
    include_exif_pii: bool,
    flickr_origin: bool,
) -> None:
    """Render the archive into a static HTML site."""
...
def fetch_external(output_dir: Path, api_key: str, what: str) -> None:
    """Download avatars and thumbnails from Flickr (optional).

    Enriches a derived archive with locally cached profile avatars and fave
    photo thumbnails so the rendered site needs no external Flickr URLs at
    browse time.  Requires a Flickr API key (--api-key or FLICKR_API_KEY).
    Run after 'derive', before 'render'.
    """
...
@click.option(
    ...
    default=0,
    type=int,
    help="Parallel workers for derive. Default: min(cpu_count, 4).",
)
@click.option("--include-private", is_flag=True, help="Serialize private views.")
@click.option("--include-private-photos", is_flag=True, help="Include non-public photos.")
@click.option(
...
def build(
    ...
    """Build a complete site from a Flickr export.

    Chains ingest → derive → render in sequence.  Pass --archive-external
    to also download avatars and thumbnails from the Flickr API before
    rendering.
    """
    resolved = _extract_with_progress(source, cache_dir or DEFAULT_CACHE) or source
    if not (resolved / "account_profile.json").is_file():
...
def inspect(output_dir: Path) -> None:
    """Show a summary of a built archive."""
    model_path = output_dir / "data" / "model.json"
    if not model_path.exists():
        _err.print(f"[red]error:[/] {model_path} not found — run 'offlickr ingest' first.")
        sys.exit(EX_DATAERR)
    ...
    external_user_nsids = {
        c.user_nsid for p in photos for c in p.comments if c.user_nsid != own_nsid
    }
    local_ids = {p.id for p in photos}
    external_photo_count = sum(
        1 for g in archive.galleries for pid in g.photo_ids if pid not in local_ids
    )
...
def serve(output_dir: Path, port: int, host: str) -> None:
    """Preview a built site on localhost."""
    os.chdir(output_dir)
