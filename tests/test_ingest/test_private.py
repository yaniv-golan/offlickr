...
def test_load_my_comments_reports_malformed_record(tmp_path):
...
def test_load_gallery_comments_reports_malformed_record(tmp_path):
...
def test_load_apps_comments_reports_malformed_record(tmp_path):
...
def test_load_set_comments_reports_malformed_record(tmp_path):
...
def test_load_group_posts_reports_malformed_record(tmp_path):
    data = {
        "discussions": [
            {
                "group_id": "g1",
    ...
    (tmp_path / "group_discussions.json").write_text(
...
def test_load_flickrmail_reports_malformed_message(tmp_path):
    data = [
        {
            "id": "msg1",
            "from": "u1",
    ...
    (tmp_path / "sent_flickrmail_part1.json").write_text(
        json.dumps(data), encoding="utf-8"
    )
    collector = IssueCollector()
...
def test_run_ingest_threads_collector_to_parsers(tmp_path):
