...
EXPECTED_PUBLIC_PHOTOS = 9
EXPECTED_TOTAL_PHOTOS = 10
EXPECTED_ALBUMS = 2
...
def test_run_ingest_emits_media_index(tmp_path: Path) -> None:
    ...
    # mini-export has 9 public photos, photo_10000005 has no media file
...
def test_include_private_populates_flickrmail(tmp_path: Path) -> None:
    run_ingest(source=MINI_EXPORT, output_dir=tmp_path,
               include_private=True, include_private_photos=False)
...
def test_include_private_populates_gallery_comments(tmp_path: Path) -> None:
    run_ingest(source=MINI_EXPORT, output_dir=tmp_path / "g",
               include_private=True, include_private_photos=False)
