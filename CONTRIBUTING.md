...
git config core.hooksPath .githooks   # install pre-push checks (one-time)
```

## Running tests, lint, and type checks

```bash
make check                            # run everything CI runs (recommended before push)
make fmt                              # auto-format + fix lint
make test                             # tests only
make lint-docs                        # markdownlint (requires Node.js)
```
